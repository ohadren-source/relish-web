...
function App() {
  ...
  const [askedSituation, setAskedSituation] = useState('')   // ← Added for "question streams first"
  ...
  async function handleGetWisdom() {
    ...
    setLoading(true)
    const submittedSituation = situation.trim()

    // === REQUESTED CHANGE: SHOW QUESTION IMMEDIATELY ===
    setAskedSituation(submittedSituation)
    setWisdom('')                    // clear previous wisdom
    ...
    try {
      ...
      alert(msg)
      setAskedSituation('')   // hide on error
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="relish-container">
      ...
      <main className="relish-content">
        ...
        {/* QUESTION APPEARS FIRST — BEFORE WISDOM LOADS */}
        {askedSituation && (
